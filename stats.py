...
all_data_filepath = os.path.join(data_path, "_reduced_100_Data_16.pkl")
...
for i in range(len(soa_names)):
    ...
    for name in data.iterkeys():
        ...
        tmp.update({name: soa.dist2heavens[i]})
    heaven_csv.append(tmp)
...
heaven_df_path = os.path.join(data_path, "_reduced_100_dist2heavens5.csv")
fft_stat_path = os.path.join(data_path, "_reduced_100_fft_stats5.csv")
